...
- [x] **Storybook** (`@storybook/nextjs-vite`) con axe per story (`addon-a11y` + addon Vitest): 13 componenti di `components/`, 33 story, più due fogli **Foundations** generati dai token (tutta la typography, tutto il color system, canonici vs alias, Categoria→accento) in light e dark. `npm run check:stories` resta manuale come `check:a11y` — vedi ADR 0012
  ...
  - [x] **`FilterPill` era il dropdown _più_ la sua didascalia**: design.md chiama Filter Pill il solo controllo, quindi "Filtra per:" e la riga che lo affianca sono passati alla pagina (nuovi slot `filter`/`filterLabel` di `articleGrid`, dove sta il resto del guscio della griglia). La story ora mostra il componente e non il pezzo di pagina attorno. Di conseguenza il componente si chiama `Dropdown` (`components/Dropdown.tsx`); "Filter Pill" resta il nome della forma in design.md e nelle ADR 0005/0006/0012, che sono verbali e non si riscrivono
  - [x] **`categoria` come enum nei Controls**: era una casella di testo libero in `CategoryTag`, `Dropdown` e `ArticleList` (la prop è `string`, docgen non può sapere altro). Due argType condivisi in `components/fixtures.ts`, con le opzioni lette da `CATEGORIE`: `categoriaArgType` (le cinque mappate + `bes`, l'accento di riserva) e `categoriaFiltroArgType` (dove `null` = nessun filtro, reso selezionabile con `mapping` perché un `<option>` non può valere null). Il `mapping` è verificato dal play di `CaricaAltri`: l'azione deve ricevere `null`
  - [x] **Story fuori da `components/`**: i `*.stories.tsx` e `fixtures.ts` (che esporta argType, quindi non è mai stato codice di componente) stanno in `stories/`, piatta: 16 file e `components/foundations/` sparisce, perché lì dentro tutto è una story e la sottocartella non serviva più. `components/` contiene solo componenti. Le story importano `@/components/X`; `stories/**` è entrato nell'`include` di `panda.config.ts` — senza quello i fogli Foundations non emettono classi e rendono senza stile — e nel glob `jsx-a11y` di `eslint.config.mjs`, per non perdere in silenzio la copertura che avevano dentro `components/`
...
- [x] Predisporre l'a11y: target WCAG 2 AA, regole `jsx-a11y` complete, contrasti a norma, focus ring tokenizzato, cambi asincroni annunciati, due controlli (`check:contrast` + `check:a11y`) — vedi ADR 0011
  ...
  - [x] **Feed RSS rimosso**: via `app/rss.xml/route.ts`, il link nel footer (che restano due, `/privacy` e `/termini`) e `getArticoliByPk()` in `lib/data.ts`, di cui la route era l'unico chiamante. Supera l'item sopra: i link del footer sono due, non tre
...
- [ ] Warning console in home: l'immagine dell'hero (`components/Hero.tsx:73`, `priority`) viene **preloaded ma non usata** — il candidato `srcset` scelto al render non è quello preloadato. Verificare `sizes` sull'`Image`
- [x] `fallow`: azzerate le 8 segnalazioni di dead code. Sei erano export pubblici usati solo nel proprio file (`montserrat`/`inter`/`greatVibes`/`montez` in `app/fonts.ts` — solo `fontVariables` esce; `PAGE_SIZE` in `lib/data.ts`; il tipo `ArticleInput` in `app/admin/actions.ts`): togliere `export` restringe l'API invece di silenziare il controllo. Due erano file raggiunti fuori dal grafo degli import (`app/__mocks__/actions.ts` via la convenzione `__mocks__` di Storybook, `scripts/shot.mjs` lanciato a mano) e portano un `fallow-ignore-file` con la motivazione
