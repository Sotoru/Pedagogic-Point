...
- [x] Predisporre l'a11y: target WCAG 2 AA, regole `jsx-a11y` complete, contrasti a norma, focus ring tokenizzato, cambi asincroni annunciati, due controlli (`check:contrast` + `check:a11y`) — vedi ADR 0011
  ...
  - [ ] Compilare `[TITOLARE]` e `[EMAIL]` in `/privacy` e `/termini` (bozze fattuali, **non** consulenza legale)
...
- [ ] Decidere se `scripts/release.sh` deve eseguire anche `npm run check:contrast` (istantaneo, zero dipendenze) e `npm run check:a11y` (richiede chromium e ~30s). Oggi il cancello di release è solo `lint` + `build`, quindi una futura modifica ai colori di `docs/design.md` non incontra la guardia se nessuno la lancia a mano
